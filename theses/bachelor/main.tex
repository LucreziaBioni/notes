...
\begin{equation}
  ...
  \end{split}
\end{equation}
According to the momentum conservation constraint in Eq. \ref{leading-order}, $E_{\max}$ must be greater than or equal to the maximum energy that a final-state parton can carry. Consequently, $E_{\max} \geq E_a$ and thus $z_{\mathrm{min}} \leq 0$. This implies that $F_{\mathrm{LM}}(z \cdot a_q) = 0$ for $z \in [z_{\mathrm{min}}, 0]$,  while the second term on the right-hand side of Eq. \ref{coll-pass-1} extends all the way down to $z_{\mathrm{min}}$. Furthermore, in $P_{qq}$, we isolate the term that is singular in the $z \to 1$ limit in the following way
\begin{equation}
...
\end{equation}
where $\gamma_q$ is the anomalous dimension of the initial-state parton $a=q$, which can be found in Appendix \ref{anomalous-dimension}, and $L_a =\log{(E_{\mathrm{max}}/E_a)}$. Therefore, the integrated soft-collinear counterterm for a gluon going collinear to a quark $a$ in the initial state reads
\begin{equation}
...
\begin{equation}
  ...
  & = \int \frac{\mathrm{d}\Omega^{d-2}}{2(2\pi)^{d-1}} 2^{-2\epsilon} \int_0^1 \frac{\mathrm{d}\eta_{i\um}}{(\eta_{i\um}(1-\eta_{i\um}))^{\epsilon}} \int [\mathrm{d}\Omega_i] \int \mathrm{d}E_\um E_\um^{1-2\epsilon} \int \mathrm{d}E_i E_i^{1-2\epsilon} \frac{g_{s,b}^2 P_{gg} F_{\mathrm{LM}} (i\um) }{(1-z) E_{i\um}^{2}\eta_{i\um}} \, .
  \end{split}
\end{equation}
We now employ the following integrals
\begin{equation}
  \int \frac{\mathrm{d}\Omega^{d-2}}{2(2\pi)^{d-1}} = \frac{1}{8\pi^2} \frac{(4\pi){\epsilon}}{\Gamma(1-\epsilon)} \, ,
\end{equation}
and  \\
...
faccio i conti, mostro che non ottengo la consueta generalised final-state anomalous dimension, ma devo sommare i casi per ottenerla. \\
...
\begin{equation}
  \begin{split}
    & P_{qq}(z) = C_F \left[\frac{1+z^2}{1-z}-\epsilon(1-z)\right]\, , \\
    & P_{gg}(z)= 2C_A \left[ \frac{z}{1-z}+\frac{1-z}{z}+z(1-z)\right] \equiv P_{qq}(1-z) \, , \\
    & P_{qg}(z) = T_R \left[1-\frac{2z(1-z)}{1-\epsilon}\right] \, , \\
...
\begin{equation}
  \begin{split}
    \hat{P}_{qq}^{(0)}(z) = C_F \left[ 2 \mathcal{D}_0 (z)-(1+z)+\frac{3}{2} \delta(1-z) \right], \qquad \qquad  \\
    \hat{P}_{qg}^{(0)}(z) = T_R \left[(1-z)^2+z^2\right] \, , \qquad \qquad\qquad\qquad\quad\qquad \, \,\\
    \hat{P}_{gq}^{(0)}(z) = C_F \left[\frac{1+(1-z)^2}{z}\right] \, , \qquad \qquad\qquad\qquad\quad\qquad \, \, \\
