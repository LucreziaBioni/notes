...
\end{equation}
where ${d}\Omega_{d-1}$ is the ($d-1$)-dimensional solid-angle element. Using the bare QCD coupling $g_{s,b}^2 = 4\pi\alpha_s$ and considering $N_p$ final-state partons at NLO, we obtain
\begin{equation}
  \langle S_\um F_{\mathrm{LM}} (\um) \rangle = \int \mathrm{d}E_\um E_\um^{d-5} \, \theta(E_{\mathrm{max}}-E_\um)  \int \frac{\mathrm{d}\Omega_{d-1}}{2 (2 \pi)^{d-1}} (-g_{s,b}^2) \sum_{(ij)}^{N_p} \frac{\rho_{ij}}{\rho_{im}\rho_{jm}} (\vec{T}_i \cdot \vec{T}_j) F_{\mathrm{LM}} \, .
\end{equation}
Recall that $\rho{ij} = 1 - \cos\theta_{ij}$, so that the dot product is $p_i \cdot p_j = E_i E_j \rho_{ij}$. We now evaluate the integrals using dimensional regularization in $d=4-2\epsilon$ dimensions. The energy integral yields a simple pole in $\epsilon$:
\begin{equation}
...
\end{equation}
where $\eta_{ij}=\rho_{ij}/2$, and ${}_2F_1$ denotes the Gauss hypergeometric function. \\
Finisco soft limit, e poi adatto esempi espliciti.

%Therefore, we can express the integrated soft-counterterm \\



In the limit where the gluon with momentum $p_3$ becomes soft, the amplitude of the process $\mathcal{P}_{1}^{\mathrm{NLO}}$ factorizes into
\begin{equation}
  S_3 F_\mathrm{LM}(1,2,3)= g_{s,b}^2 2C_F \frac{\rho_{12}}{E_3^2\rho_{13}\rho_{23}} F_\mathrm{LM}(1,2)
\end{equation}
where $g_{s,b}$ is the bare QCD coupling, $C_F=\frac{4}{3}$ is the QCD color factor (see Eq. \ref{casimir-operators}), and $F_\mathrm{LM}(\dots)$ denotes the corresponding parton-level matrix element squared, as defined in Eq.\ref{flm}, with the arguments specifying the partons involved in the process. The integration of this term over the phase space of the unresolved gluon gives
\begin{equation}
  \langle S_3 F_\mathrm{LM}(1,2,3) \rangle = \frac{2 C_F [\alpha_s] \mu^{2\epsilon}}{\epsilon^2}\frac{\Gamma(1-\epsilon)^2}{\Gamma(1-2\epsilon)} (2E_\mathrm{max})^{-2\epsilon} F_\mathrm{LM}(1,2),
\end{equation}
...
\begin{equation}
  \langle S_3 F_\mathrm{LM}(1,2,3) \rangle = \frac{1}{3} \frac{2 C_A [\alpha_2]}{\epsilon^2} \left(\frac{m_H^2}{\mu^2}\right)^{-\epsilon} \eta_{12}^{-\epsilon}[1+\epsilon^2(\mathrm{Li}_2(1-\eta_{12})-\zeta_2)+\mathcal{O}(\epsilon^2)]F_\mathrm{LM}(1,2),
\end{equation}
where $\eta_{ij}=\rho_{ij}/2$. \\
