...
One of the most interesting features of the NSC subtraction scheme is that the subtractions for a generic process, no matter how complicated, are built from a relatively small set of basic ingredients. Indeed, the soft contributions are straightforward to compute, while, for the nested-soft-collinear ones, we only need to consider pairs of partons. This leads to three possible cases: both partons are initial state, both are final state, or one is initial and one is final. These three possibilities have been studied in \cite{Caola:1902}, \cite{Caola:1907}, and \cite{Asteriadis:1910}, respectively. \\
In general, for each of the processes under consideration, corrections from different partonic channels should be taken into account. In the following discussion, however, we will focus on one contribution per process. This is because our current aim is to present only the main results from the cited works, in order to illustrate the functioning of the NSC subtraction scheme. Furthermore, as previously mentioned, we will highlight only the form of the real-emission NLO corrections in the soft and collinear limits. \\
Ref. \cite{Caola:1902} provides the analytic formulas for the production of a generic color-singlet final state $V$ at a hadron collider, i.e., $q_1\bar{q}_2 \to V$ (which we will call $\mathcal{P}_{1,0}$), such as the Drell-Yan process, illustrated in the following diagram
\begin{equation*}
    \mathcal{P}_{1,0}: \quad q_1 + \bar{q}_2 \to e^+ + e^- \qquad \qquad
  ...
  \end{tikzpicture}
\end{equation*}
The real emission contribution refers to the process $\mathcal{P}_{1,1}: q_1\bar{q}_2 \rightarrow V+ g_3$, that, in the case of Drell-Yan, can be represented as
\begin{equation*}
    \mathcal{P}_{1,1}: \quad q_1 + \bar{q}_2 \to e^+ + e^- + g_3 \qquad \qquad
  \begin{tikzpicture}[baseline = (current bounding box.center), thin]
    \begin{feynman} [inline = (r.base)]
      % Definizione di TUTTI i vertici
  ...
  \end{tikzpicture}
\end{equation*}
Instead, Ref. \cite{Caola:1907} considers the decay of a color-singlet particle $Q$ into quarks and gluons, and, in particular, the process $\mathcal{P}_{2,0}: Q \to f_i f_j + X$, where ${f_i,f_j}$ can be either ${g,g}$ or ${q,\bar{q}}$. We will focus on the discussion of the decay to a $gg$ final state since its singularity structure is more complex: once the corrections for $Q \to gg$ are understood, the corrections for $Q\to q \bar{q}$ are easily derived. This process, if for instance $Q$ is the Higgs boson, is represented by the following diagram
\begin{equation*}
    \mathcal{P}_{2,0}: H \to g_1 + g_2 \qquad \qquad
    \begin{tikzpicture}[baseline = (current bounding box.center), thin]
    \begin{feynman} []
      \vertex (a) {\(H\)};                   
...
\begin{equation*}
  \mathcal{P}_{2,1}: H \to g_1 + g_2 + g_3\qquad
  \begin{tikzpicture}[baseline=(current bounding box.center), scale=1.0, every node/.style={font=\small}]
    \begin{feynman}
      % Higgs
      \vertex (H) at (-2,0) {\(H\)};
  ...
  \end{tikzpicture}
\end{equation*}
Finally, Ref. \cite{Asteriadis:1910} provides the NLO QCD corrections for the deep-inelastic scattering process of an electron on a proton. At leading order, this receives contributions from both the process $\mathcal{P}_{3,0}: q_1 + e^-_2 \to e^-_3 + q_4$ and $\hat{q} + e^- \to e^- + \hat{q}$. However, since these are identical, we will consider only the first one, which is illustrated in the following diagram
\begin{equation*}
    \mathcal{P}_{3,0}: q_1 + e^-_2 \to e^-_3 + q_4 \qquad \qquad
    \begin{tikzpicture}[baseline = (current bounding box.center), thin]
    \begin{feynman} [inline = (r.base)]
      \vertex (a) {};                   
  ...
  \end{tikzpicture}
\end{equation*}
The real emission contribution at NLO refers to the process $\mathcal{P}_{3,0}: q_1 + e^-_2 \to e^-_3 + q_4 + g_5$, where the gluon becomes collinear to the incoming or the outcoming quark

\begin{equation*}
\mathcal{P}_{3,1}: q_1 + e^-_2 \to e^-_3 + q_4 + g_5 \qquad
