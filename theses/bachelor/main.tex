...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
If we also include the soft limit, we obtain the soft-collinear integrated counterterm, following a procedure analogous to that outlined in Section~\ref{initial-state-section}
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  \begin{split}
  \langle C_{i\mathfrak{m}}^{\theta_i} \bar{S}_{\mathfrak{m}}^{\theta_s} \Delta^{\mathfrak{m}} F_{\mathrm{LM}}(\mathfrak{m})\rangle &= -[\alpha_s] \,[\mathrm{d}\eta_{i\um \, \theta_i}] \, \bigl< \left(\frac{2E_i}{\mu}\right)^{-2\epsilon} \gamma^{2,2}_{z,g \to g, g} \, F_{\mathrm{LM}} \bigr> \, , 
  \end{split}
\end{equation}
