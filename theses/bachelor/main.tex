...
\end{equation}
where $F(x)$ is an arbitrary function regular at $x=0$. The integrand diverges at the lower limit, and this singularity is regulated by the parameter $\epsilon$, which uktimately produces a $1/\epsilon$ pole upon integration. We want to isolate this pole analitically, and define the integral in such a way that the limit $\epsilon \to 0$ can be taken safely. To achieve this, we write $F(x)=[F(x)-F(0)]+F(0)$
\begin{equation}
...
Now, suppose we have a function $\mathcal{S}$ that reproduces the leading singular behavior of $F^{ab}_{\mathrm{LM}}$ (defined in Eq. \ref{flm}) in all soft and collinear limits. Let $\um$ be the unresolved parton; we can then write the real-emission cross section as
\begin{equation}
...
The second term involves only the unresolved parton $\um$, making it possible to integrate over its phase space without affecting the jet observables. Furthermore, this integration enables the extraction of $1/\epsilon$ poles, which describe the singular behavior of $\mathcal{S}$ and, consequently, of the amplitude itself. This allows for the cancellation of these poles with those arising from the virtual contributions.




\section{Soft and Collinear operators}
differenza tra vari approcci è scelta di S \\
grazie alla fattorizzazione mostrata nel paragrafo \ref{properties}, S = soft limit di F o collinear limit di F, e si fattorizza in parte che non dipende da unresolved parton e parte che vi dipende e la cui integrazione porta a esplicitare i poli.
