...
A theoretical description of hadron collisions is complicated primarily by our limited knowledge of the strong force, which binds the elementary constituents of hadrons. Strong interactions are described by Quantum Chromodynamics (QCD), a non-Abelian gauge theory based on the SU(3) symmetry group. The QCD Lagrangian is not analytically solvable, making it extremely difficult to understand proton dynamics from first principles. A way to overcome these obstacles becomes manifest when we consider how hadrons collide at high energies \cite{Asteriadis2020}. Typically, they undergo either elastic scattering or diffractive dissociation: in the first case, they collide but remain intact; in the second, they disintegrate into a small number of hadrons. However, on rare occasions, a more interesting phenomenon can occur: the elementary partons that compose the hadrons can interact and exchange a large amount of momentum. This is the case in so-called ``hard scattering processes," which are central to probing the electroweak scale, the Higgs boson, and potential new physics. Their importance is directly related to a key feature of the strong force: asymptotic freedom. The essence of this property is the weakening of the colour force at short distances \cite{ellis}. In this high-energy regime, the interacting partons can be approximated as being nearly free, which permits a perturbative description of the strong interaction. The strength of these interactions is governed by the coupling constant $\alpha_{\text{S}}$. Due to asymptotic freedom, $\alpha_{\text{S}}$ is on the order of $0.1$ at energies relevant for contemporary colliders\footnote{This value is energy-dependent; $\alpha_{\text{S}}\sim0.1$ is typical at energies around 100 GeV.}. This makes the strong force about ten times stronger than the electromagnetic interaction (whose coupling constant is $\alpha\sim1/137$), yet sufficiently small to allow for the application of well-defined perturbative approximation methods.

The relevance of hard processes stems from a fundamental property of QCD: asymptotic freedom \cite{ellis}. This phenomenon describes the weakening of the colour force at short distances. As a result, partons in high-energy collisions behave as nearly free particles, and their interactions are characterized by a small coupling constant $\alpha_{\text{S}}$, which is on the order of 0.1 at modern collider energies\footnote{This value is energy-dependent; $\alpha_{\text{S}}\sim0.1$ is typical around 100 GeV.}. Although the strong force remains about ten times stronger than the electromagnetic interaction (where $\alpha \sim 1/137$), this modest value of $\alpha_{\text{S}}$ makes a perturbative description possible, enabling the application of well-defined approximation methods.
...
\section{The origin of infrared divergences in real-emission contributions}
\label{origin-divergences}
...
The desired method necessitates the isolation of singularities in the real corrections without integrating over the momenta of any of the final-state particles, in order to keep the kinematics of all the final-state particles intact. Thanks to the fact that soft or collinear radiated partons are unresolved and that infrared-safe observables are not altered by the presence of unresolved partons, we can integrate over the phase space of the unresolved partons without losing information about the final state. This element is the key that gives meaning to subtraction schemes. \\
Suppose we have a function $\mathcal{S}$ that reproduces the leading singular behavior of $F^{ab}_{\mathrm{LM}}$ (defined in Eq. \ref{flm}) in all soft and collinear limits. Let $\um$ be the unresolved parton; we can then write the real-emission cross section as
\begin{equation}
...
The second term involves only the unresolved parton $\um$, making it possible to integrate over its phase space without affecting the jet observables. Furthermore, this integration enables the extraction of $1/\epsilon$ poles, which describe the singular behavior of $\mathcal{S}$ and, consequently, of the amplitude itself. This allows for the cancellation of these poles with those arising from the virtual contributions. To understand how this mechanism operates, consider the integral
\begin{equation}
    I = \int_0^1 \mathrm{d}x \frac{1}{x^{1+\epsilon}} F(x)
\end{equation}
where $F(x)$ is an arbitrary function regular at $x=0$ \\
In our specific case, the function $F(x)$ coincides with the lower-multiplicity $F^{ab}_{\mathrm{LM}}$ function, due to the factorization in the soft and collinear limits illustrated in Sec. \ref{origin-divergences}. This approach, therefore, resolves the problem of the dimensional mismatch between the integration domains of the virtual and real corrections.
